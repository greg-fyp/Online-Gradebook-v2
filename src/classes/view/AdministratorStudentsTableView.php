...
class AdministratorStudentsTableView extends AdministratorView {
	public function __construct() {
		parent::__construct();
	}

	public function create() {
		$this->page_title = APP_NAME . ' Administrator Students View';
		$this->createPage();
	}
		...
		$this->html_output .=  <<< HTML
			...
			<div class='modal fade' id='more-modal' tabindex='-1' role='dialog' aria-labelledby='More information' aria-hidden='true'>
				...
				function continueDelete() {
					location.href = '$target_file?route=drop_student&id=' + toDelete;
				}
			</script>
		HTML;
	}
	...
	private function buildTable() {
		...
		foreach ($this->personal_details['students'] as $item) {
			...
			$password = $item['user_hashed_password'];
			$gender = $item['user_gender'];
			$dob = $item['user_dob'];
			$address = $item['user_address'];
			$added = $item['student_added_timestamp'];
